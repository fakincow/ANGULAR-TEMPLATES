...
import { StabDataService } from '../services/stab-data-service';
@Component({
...
export class RoomsLobbyComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private router: Router, private audioService: AudioService, private stabDataService : StabDataService) { }
  ...
  fetchSettings() {
    ...
    */
    let lobbyData =  this.stabDataService.getLobbySlotDats();
    this.slides = this.fixMissing(lobbyData.slots);
    this.slides.sort(this.compare);
    setTimeout(() => { this.settingsLoaded = true }, 1500);
  }
  ngOnInit() {
